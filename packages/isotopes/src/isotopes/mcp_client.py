"""MCP client integration for isotopes.

Loads tools from MCP (Model Context Protocol) servers and converts them
to isotopes-core Tool objects so they can be used in agent loops.
...
def _ensure_mcp() -> None:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install it with: pip install isotopes[mcp]"
        ) from None
...
class McpToolLoader:
    ...
    async def load_from_server(self, server_config: dict[str, Any]) -> list[Any]:
        ...
        Returns:
            A list of isotopes-core ``Tool`` objects backed by the MCP server.
    ...
    async def _load_stdio(self, config: dict[str, Any]) -> list[Any]:
        ...
        async with stdio_client(params) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                return [
                    self._mcp_tool_to_isotopes_tool(t, session) for t in response.tools
                ]
    ...
    def _mcp_tool_to_isotopes_tool(self, mcp_tool: Any, client: Any) -> Any:
        ...
        Returns:
            An isotopes-core ``Tool`` instance.
        """
