"""IsotopeAgent — high-level agent wrapping isotopes-core with presets."""
...
from isotopes_core.types import UserMessage, AgentEvent, TurnEndEvent, TextContent

from isotopes.compaction import (
    CompactionResult,
    compact_messages,
    _estimate_messages_tokens,
)
from isotopes.presets import Preset, get_preset
...
class IsotopeAgent:
    """High-level agent that wraps isotopes-core Agent with preset support.
    ...
    def _patch_tool_workspaces(self, tools: list[Tool]) -> None:
        ...
        import isotopes.tools.grep as grep_mod
        import isotopes.tools.glob as glob_mod
        import isotopes.tools.read as read_mod
    ...
    async def compact(self) -> CompactionResult:
        ...
        if result.summary and result.messages_compacted > 0:
            ...
            if self._session_store and self._session_id:
                from isotopes.session import SessionEntry
    ...
    @property
    def core(self) -> Agent:
        """The underlying isotopes-core Agent (for advanced use)."""
        return self._agent
