...
from isotopes.session import SessionMeta
...
class TestSessionsCommand:
    ...
    @patch("isotopes.cli.SessionStore")
...
class TestMainDispatch:
    ...
    @patch("isotopes.cli.list_sessions")
    ...
    @patch("isotopes.cli.run_one_shot", new_callable=AsyncMock)
...
class TestRunRpc:
    ...
    @patch("isotopes.cli.asyncio")
    @patch("isotopes.cli.RpcServer")
    @patch("isotopes.cli.IsotopeAgent")
    @patch("isotopes.cli.get_preset")
    @patch("isotopes.cli.create_provider")
    @patch("isotopes.cli.load_config")
...
class TestCLIIntegration:
    ...
    def test_cli_help_subprocess(self) -> None:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "isotopes", "--help"],
            capture_output=True,
            text=True,
        )
    ...
    def test_cli_version_subprocess(self) -> None:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "isotopes", "--version"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0
    ...
    def test_cli_no_command_defaults_to_chat(self) -> None:
        ...
        from isotopes.cli import create_parser
