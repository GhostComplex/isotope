...
from isotopes.config import McpServerConfig, load_config
from isotopes.mcp_client import McpToolLoader, _ensure_mcp
...
# ---------------------------------------------------------------------------
# _mcp_tool_to_isotopes_tool
# ---------------------------------------------------------------------------


class TestMcpToolConversion:
    """Tests for McpToolLoader._mcp_tool_to_isotopes_tool."""
    ...
    def test_basic_conversion(self) -> None:
        ...
        tool = loader._mcp_tool_to_isotopes_tool(mcp_tool, client)
...
class TestLoadFromServer:
    ...
    async def test_invalid_config_raises_value_error(self) -> None:
        ...
        loader = McpToolLoader()
        with patch("isotopes.mcp_client._ensure_mcp"):
    ...
    async def test_stdio_transport(self) -> None:
        ...
        with (
            patch("isotopes.mcp_client._ensure_mcp"),
            patch.dict(
                sys.modules,
                {
                    "mcp": mock_mcp,
...
class TestMcpConfig:
    ...
    def test_default_config_has_empty_mcp_servers(self) -> None:
        ...
        from isotopes.config import IsotopeConfig
