# isotopes-core — Follow-up: Integration Tests & TUI
...
A standalone interactive TUI at the repo root (NOT inside `src/isotopes_core/`).
...
**Dependencies:** Only `isotopes-core` itself + `httpx` (for model listing) + optionally `rich` for pretty output. NO `textual` or heavy TUI frameworks — keep it simple stdin/stdout.

**Example session:**
```
isotopes-core TUI v0.1
...
`feat/isotopes-core/dev-followup`

## Technical Constraints

- Integration tests must be skippable (don't break CI without proxy)
- TUI must NOT be part of the `isotopes-core` package (not in `src/`)
