...
class TestOptionalProviderImports:
    ...
    def test_openai_not_installed(self) -> None:
        ...
        modules_to_hide = {
            k: v for k, v in sys.modules.items() if "openai" in k or "isotopes_core.providers" in k
        }
        with patch.dict(sys.modules, {**{k: None for k in modules_to_hide}}):
            ...
            for key in list(sys.modules.keys()):
                if key.startswith("isotopes_core.providers"):
            ...
            mod = importlib.import_module("isotopes_core.providers")
    ...
    def test_anthropic_not_installed(self) -> None:
        ...
        modules_to_hide = {
            ...
            if "anthropic" in k or "isotopes_core.providers" in k
        }
        with patch.dict(sys.modules, {**{k: None for k in modules_to_hide}}):
            for key in list(sys.modules.keys()):
                if key.startswith("isotopes_core.providers"):
    ...
    def test_proxy_not_installed(self) -> None:
        ...
        modules_to_hide = {
            k: v
            for k, v in sys.modules.items()
            if "openai" in k or "isotopes_core.providers.proxy" in k
        }
        with patch.dict(sys.modules, {**{k: None for k in modules_to_hide}}):
            for key in list(sys.modules.keys()):
                if key.startswith("isotopes_core.providers"):
    ...
    def test_base_always_available(self) -> None:
        ...
        from isotopes_core.providers import (
            CircuitState,
            Provider,
            RetryConfig,
            RouterProvider,
